...
export default function EksamenPage() {
  ...
  const dateDisplay = examDate
    ? new Date(examDate + "T12:00:00").toLocaleDateString("nb-NO", { day: "numeric", month: "short" })
    : null;

  return (
    <AppShell>
      <div style={{ maxWidth: "560px", margin: "0 auto", fontFamily: "Inter, system-ui, sans-serif", display: "flex", flexDirection: "column", height: "100%" }}>

        {/* ── Header ── */}
        <div style={{ padding: "16px 0 10px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <h1 style={{ fontFamily: "Syne, system-ui, sans-serif", fontWeight: 800, fontSize: "22px", letterSpacing: "-0.5px", color: "var(--text)", margin: 0 }}>
            Eksamen
          </h1>
          {/* Tappable date chip */}
          <button
            ...
            style={{
              ...
              padding: "7px 12px", borderRadius: "var(--r-full)", border: "none",
              backgroundColor: examDate ? "var(--accent)" : "var(--bg-alt)",
              color: examDate ? "#fff" : "var(--text-muted)",
          ...
          >
            ...
            {dateDisplay
              ? daysLeft === 0 ? "I dag!" : `${dateDisplay} · ${daysLeft}d`
              : "Sett dato"}
          </button>
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: "10px", paddingBottom: "8px" }}>
          ...
          <div style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", padding: "10px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            <p style={{ fontSize: "10px", fontWeight: 700, color: "var(--ink-light)", letterSpacing: "0.9px", textTransform: "uppercase", marginBottom: "8px", paddingLeft: "2px" }}>Fag</p>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      gap: "3px", padding: "7px 2px", borderRadius: "8px", border: "none",
                      backgroundColor: active ? "var(--text)" : "var(--bg-alt)",
                      color: active ? "var(--bg)" : "var(--text)",
                      fontFamily: "Inter, system-ui, sans-serif", fontSize: "10px", fontWeight: active ? 700 : 400,
                      cursor: "pointer", transition: "all 0.12s",
                      WebkitTapHighlightColor: "transparent",
                      textAlign: "center", lineHeight: 1.15, minHeight: "46px",
                    }}
                  >
                    <SubjectIcon id={s.id} size={14} color={active ? "var(--bg)" : color} />
                    <span style={{ maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingInline: "2px" }}>{s.short}</span>
                  </button>
                );
              })}
            </div>
          ...
          {activeFag.variants && (
            <div style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", padding: "10px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, color: "var(--ink-light)", letterSpacing: "0.9px", textTransform: "uppercase", marginBottom: "8px", paddingLeft: "2px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5px" }}>
                {activeFag.variants.map(v => (
                  ...
                  >
                    {v.label}
                    <span style={{ display: "block", fontSize: "11px", fontWeight: 400, opacity: 0.7, marginTop: "1px" }}>{v.desc}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div style={{
            ...
            border: `1px solid ${drawnTopic ? "transparent" : "var(--border)"}`,
            borderRadius: "var(--r-lg)", padding: "20px 16px",
            textAlign: "center", transition: "background 0.35s ease",
            display: "flex", flexDirection: "column",
            alignItems: "center", justifyContent: "center", gap: "8px",
            minHeight: "96px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)",
          }}>
            {drawing ? (
              <div style={{ display: "flex", gap: "5px" }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: "7px", height: "7px", borderRadius: "50%", backgroundColor: "var(--accent)", animation: `pulse 0.8s ${i * 0.15}s infinite` }} />
                ))}
              </div>
            ) : drawnTopic ? (
              <>
                <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.45)", letterSpacing: "0.8px", textTransform: "uppercase", margin: 0 }}>Trukket tema</p>
                <p style={{ fontFamily: "Syne, system-ui, sans-serif", fontWeight: 800, fontSize: "20px", color: "#fff", letterSpacing: "-0.3px", margin: 0, lineHeight: 1.2 }}>{drawnTopic}</p>
                <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", margin: 0 }}>{activeLabel}</p>
              </>
            ) : (
              <p style={{ fontSize: "13px", color: canDraw ? "var(--text-muted)" : "var(--ink-light)", margin: 0, lineHeight: 1.4 }}>
                {canDraw ? "Trykk for å trekke et tilfeldig tema" : `Velg hvilken ${activeFag.label} du har eksamen i`}
              </p>
            )}
          </div>
        ...
        {/* ── Sticky CTA ── */}
        <div style={{ flexShrink: 0, paddingTop: "8px", paddingBottom: "calc(8px + env(safe-area-inset-bottom))", backgroundColor: "var(--bg)" }}>
          {drawnTopic ? (
            ...
            <button
              ...
              style={{
                ...
                boxShadow: canDraw ? "0 4px 16px rgba(0,0,0,0.14)" : "none",
                opacity: drawing ? 0.6 : 1, transition: "all 0.15s",
                WebkitTapHighlightColor: "transparent",
              }}
            >
